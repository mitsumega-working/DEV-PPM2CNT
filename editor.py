...
class EDITOR(EDITOR_templates):
  def __init__(self, inp_path, exepath):
    ...
    self.editing_files = set()  # 編集中ファイルのセット
    self.version = 'Ver: 1.0\n2025.07.11'
    EDITOR_templates.__init__(self, exepath)
...
#### making tab2
  def make_setting(self):
    ...
    def dir_read(): self.exepath = filedialog.askdirectory(initialdir=self.exepath)
...
#### buttons
  def exe_quit_button(self):
    ...
    quitbtn = ttk.Button(frame_cvtbtn, text="閉じる",command=sys.exit)

    cvtbtn.pack(fill = "x", padx=5, side = "left", pady=10)
    quitbtn.pack(fill = "x", padx=5, side = "right", pady=10)

if __name__ == "__main__":
  editor = EDITOR(r'Analysis-1.inp', r'C:\Users\toru\Desktop\DEV-PPM2CNT\FrontISTR-DEV250506')
  editor.make_window()
